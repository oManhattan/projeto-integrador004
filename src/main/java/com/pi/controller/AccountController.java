...
public class AccountController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest request, HttpServletResponse httpResponse) {
        try {
            ...
            if (response.getA() instanceof ProfissionalEntity) {
                ProfissionalResponse profissional = ProfissionalConverter.toResponse((ProfissionalEntity) response.getA());
    ...
    public ResponseEntity<?> alterarSenhaComEmail(@RequestBody EmailRequest email) {
        try {
            accountService.newPasswordRequest(email.getEmail());
